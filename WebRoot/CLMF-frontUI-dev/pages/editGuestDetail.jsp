...
<html lang="en">
	...
	<body>
		<div class="modal-content">
			<form action="${contextPath}/guest_infor/submitEditGuestDetail" method="post">
			<div class="modal-header" style="background-color: rgba(30,40,80,0.8); height: 50px;">
				...
				<div class="pull-right btn-group-sm ">
					<button type="submit" class="btn btn-success">保存<span class="fa fa-save"></span></button>
			...
			<div class="modal-body">
				...
				<div id="myTabContent1" class="tab-content">
					<input type="hidden" name="guestInfor.id" value="${guestInfor.guest_id}"/>
						...
						<table class="table  table-hover table-striped table-responsive">
							<tbody>
								...
								<tr>
									...
									<td>国家地区：</td>
									<td>
										<select id="guest_country_email" name="">
								...
								<tr>
									...
									<td>国籍：</td>
									<td><input type="text" placeholder="请输入国籍" style="width: 100%;" ></td>
								</tr>
								<!---电子邮件结束--->
									...
									<td>
										<c:choose>
											<c:when test="${guestInfor.visa_sex == 1}">
												<label class="mr20"><input type="radio" name="visa.sex" value="1" checked="checked" />男</label>
												<label><input type="radio" name="visa.sex" value="2" />女</label>
											</c:when>
											<c:when test="${guestInfor.visa_sex == 2}">
												<label class="mr20"><input type="radio" name="visa.sex" value="1" />男</label>
												<label><input type="radio" name="visa.sex" value="2" checked="checked" />女</label>
											</c:when>
											<c:otherwise>
												<label class="mr20"><input type="radio" name="visa.sex" value="1" />男</label>
												<label><input type="radio" name="visa.sex" value="2" />女</label>
										...
										<div>
											...
											<c:forEach items="${routeList}" var="route">
												<c:if test="${route.route_start_time == '2015-08-24'}">
													...
													<c:choose>
														<c:when test="${flag == true}">
															<p class="ml20"><label><input type="radio" checked="checked" name="routeId24" value="${route.id}">&nbsp;${route.route_nicName}：${route.route_content}(${route.route_name})-(${route.route_start_time})-限定人数(${route.limit_num})-已报名人数(${route.regist_num})</label></p>
														</c:when>
														<c:otherwise>
															<p class="ml20"><label><input type="radio" name="routeId24" value="${route.id}">&nbsp;${route.route_nicName}：${route.route_content}(${route.route_name})-(${route.route_start_time})-限定人数(${route.limit_num})-已报名人数(${route.regist_num})</label></p>
														</c:otherwise>
													</c:choose>
												</c:if>
											</c:forEach>
										...
										<div>
											...
											<c:forEach items="${routeList}" var="route">
												<c:if test="${route.route_start_time == '2015-08-25'}">
													...
													<c:choose>
														<c:when test="${flag == true}">
															<p class="ml20"><label><input type="radio" checked="checked" name="routeId25" value="${route.id}">&nbsp;${route.route_nicName}：${route.route_content}(${route.route_name})-(${route.route_start_time})-限定人数(${route.limit_num})-已报名人数(${route.regist_num})</label></p>
														</c:when>
														<c:otherwise>
															<p class="ml20"><label><input type="radio" name="routeId25" value="${route.id}">&nbsp;${route.route_nicName}：${route.route_content}(${route.route_name})-(${route.route_start_time})-限定人数(${route.limit_num})-已报名人数(${route.regist_num})</label></p>
														</c:otherwise>
													</c:choose>
												</c:if>
											</c:forEach>
						...
						<table class="table table-striped  table-hover table-responsive">
							...
							<tbody>
								...
								<tr>
									...
									<td id="room_type" class="text-left">
									...
									<td class="text-left">
										<c:choose>
											<c:when test="${guestInfor.hotel.is_smoking == 0}">
												<label><input type="radio" name="is_smoking" value="1"/>吸烟楼层 </label>
												<label><input type="radio" name="is_smoking" value="0" checked="checked"/>不吸烟楼层</label>
											</c:when>
											<c:when test="${guestInfor.hotel.is_smoking == 1}">
												<label><input type="radio" name="is_smoking" value="1" checked="checked"/>吸烟楼层 </label>
												<label><input type="radio" name="is_smoking" value="0"/>不吸烟楼层</label>
											</c:when>
											<c:otherwise>
												<label><input type="radio" name="is_smoking" value="1"/>吸烟楼层 </label>
												<label><input type="radio" name="is_smoking" value="0"/>不吸烟楼层</label>
								...
								<tr>
									<td>同宿人员安排：</td>
									<td class="chose-accomadates text-left">
										<c:choose>
									...
									<td colspan="2">
										<table class="table table-striped  table-hover table-responsive">
											<tr>
												...
												<td>
													<input type="text" name="roommate_name" placeholder="请输入同宿人员姓名" value="${guestInfor.hotel.roommate_name}"  width="80%"/>
												</td>
											</tr>
											...
											<tr>
												...
												<td>
													<input type="text" name="roommate_idNo" placeholder="请输入同宿人员身份证号" value="${guestInfor.hotel.roommate_idNo}" width="80%"/>
												</td>
											</tr>
											<tr>
												<td colspan="2" class="text-center">
													<button type="button" class="btn btn-primary">验证同宿人员</button>
												</td>
											</tr>
											<%-- <tr>
												<td>同宿人员电子邮件：</td>
												<td>
													<label><input type="roommate_email" name="${guestInfor.hotel.roommate_email}"></label>
												</td>
								...
								<tr>
									...
									<td colspan="3" class="ifNeedDevice text-left">
										<label class="mr20"><input type="radio" name="needDevice" value="0" checked="checked">否</label>
